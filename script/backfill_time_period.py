...
# @Author  : yuanzi
# @Time    : 2026/04/05
# Description: 历史数据回填 time_period 字段
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('backfill_time_period.log')
    ]
)
...
class ProgressTracker:
    ...
    def __init__(self, progress_file='backfill_progress.json'):
...
class TimePeriodBackfill:
    ...
    def get_time_period(self, timestamp_str, node):
        """
        根据时间戳和节点类型判断流量所属时段
        与 resolver.py 中的逻辑保持一致
        """
        try:
    ...
    def prepare_bulk_actions(self, docs):
        ...
        for doc in docs:
            ...
            time_period = self.get_time_period(timestamp, node)
...
def main():
    parser = argparse.ArgumentParser(description='历史数据 time_period 字段回填工具')
    parser.add_argument('--index', '-i', nargs='+', default=['sflow-*', 'ipbandwidth-*', 'ipbw-*'],
                       help='要处理的索引模式 (支持多个)')
    parser.add_argument('--host', default='http://localhost:9200', help='Elasticsearch 地址')
    parser.add_argument('--user', default='nettraffic_analyzer', help='Elasticsearch 用户名')
    parser.add_argument('--password', default='nettraffic_analyzer', help='Elasticsearch 密码')
    parser.add_argument('--batch-size', type=int, default=5000, help='每批处理的文档数量')
    ...
    # 如果需要重置进度
    if args.reset:
        if os.path.exists('backfill_progress.json'):
            os.remove('backfill_progress.json')
            logger.info("进度已重置")

    # 创建回填工具
    backfill = TimePeriodBackfill(
        es_host=args.host,
        es_user=args.user,
